...
p2l.rawDataEMG = p2l.proj + "Rawdata" + fs + "EMG" + fs + subj + fs;
p2l.results = p2l.proj + "Results" + fs + "MocapTreadmill" + fs + "bySubj" +fs + subj + fs;
p2l.resultsEMG = p2l.proj + "Results" + fs + "EMG" + fs + "bySubj" +fs + subj + fs;
p2l.checks = p2l.proj + "Checks" + fs + "MocapTreadmill" + fs + subj + fs;
p2l.checksEMG = p2l.proj + "Checks" + fs + "EMG" + fs + subj + fs;
...
if ~isfolder(p2l.resultsEMG), mkdir(p2l.resultsEMG); end
if ~isfolder(p2l.checks), mkdir(p2l.checks); end
if ~isfolder(p2l.checksEMG), mkdir(p2l.checksEMG); end
...
for t = trialTypes
    ...
    f2l.emg.(t) = p2l.rawDataEMG + subj + "_" + t + "_emg.txt";
end
...
%% Truncate data 
%
% t_start = when condition begins aka treadmill play
%
% t_beg = From mocap record -> treadmill play
%
% Frame_Start = From treadmill record -> MoCap/EMG record: FrameNumber = 0 
%
% to get difference between hitting record on MoCap/EMG to t_start:
% 
...
for t = trialTypes
    ...
    MC.(t).t_start = MC.(t).time0(MC.(t).t_start_idx); %check t_start

    %find frame_start
    MC.(t).Frame_start_idx = find(MC.(t).data.FrameNumber < 0.0001, 1, 'last'); %index when MoCap/EMG record clicked
    MC.(t).Frame_start = MC.(t).time0(MC.(t).Frame_start_idx);

    TM.(t).t_beg = TM.(t).t_start -  MC.(t).Frame_start;

    %save copy of rawdata
...
for t = trialTypes
    figure
    plot(EMG.(t).rawdata.Times, EMG.(t).rawdata.BICEPSFEMLTuV, 'm')
    hold on
    plot(EMG.(t).data.Times, EMG.(t).data.BICEPSFEMLTuV, 'g')
    title(t)
    legend("rawdata","truncated")
end 
...
for t = trialTypes
    ...
    MC.(t).idata = array2table(interp1(MC.(t).time0, MC.(t).data{:,:},timeq));
    ...
    subplot(1,2,1)
    plot(MC.(t).time0,MC.(t).data.FP1_ForY, 'b', 'LineWidth',2)
    hold on
    plot(MC.(t).itime, MC.(t).idata.FP1_ForY,'r:','LineWidth',2);
    legend("original","interpolated")

    % interpolate treadmill data
    TM.(t).idata = array2table(interp1(TM.(t).time0,TM.(t).data{:,:},timeq));
    ...
    subplot(1,2,2)
    plot(TM.(t).time0,TM.(t).data.SpeedActual1, 'b', 'LineWidth',2)
    hold on
...
%     strides2plot = find(fix(min(strides.TM.(t).normdata.SwayActual)*100)~=0);
    ...
    strides.(t).pre = find(MC.(t).itime(GE.(t).events.RHS)> (10 + TM.(t).t_start) & MC.(t).itime(GE.(t).events.RHS) < (70 + TM.(t).t_start));
    strides.(t).pert = find(MC.(t).itime(GE.(t).events.RHS)>(70 + TM.(t).t_start) & MC.(t).itime(GE.(t).events.RHS) < (370 + TM.(t).t_start));
    strides.(t).post = find(MC.(t).itime(GE.(t).events.RHS)> (370 + TM.(t).t_start) & MC.(t).itime(GE.(t).events.RHS) < (430+ TM.(t).t_start));
...
%         patch(patch_x, [varmean+varstd; flipud(varmean-varstd)], 'k', 'EdgeColor', 'k', 'facealpha', 0.5, 'edgealpha', 0.5);
        ...
        WSPD.(t).mean(:,1) = mean(wspddata(:,strides.(t).pre),2);
        WSPD.(t).std(:,1) = std(wspddata(:,strides.(t).pre),0,2);
        WSPD.(t).mean(:,2) = mean(wspddata(:,strides.(t).pert),2);
        WSPD.(t).std(:,2) = std(wspddata(:,strides.(t).pert),0,2);
        WSPD.(t).mean(:,3) = mean(wspddata(:,strides.(t).post),2);
        WSPD.(t).std(:,3) = std(wspddata(:,strides.(t).post),0,2);
...
%     xlabel("time (s)"); ylabel("belt speed")
    ...
    % ~~ END FILTER DATA ~~
    SK.(t) = calc_stepkinematics(MC.(t).markers.filt,GE.(t).events(GE.(t).goodstrides,:),T,des_srate);
...
% TM.(t).data.t(time_idx)
% ptime = TM.(t).checks.freq.onset_time + TM.(t).t_start;
% itime = TM.(t).itime;
% gaitevents_perts(vGRF_filt,GE.(t).events(GE.(t).goodstrides,:),itime, ptime, 0,subj+", "+t)
% 
% hold on
