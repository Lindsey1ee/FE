...
%     strides2plot = find(fix(min(strides.TM.(t).normdata.SwayActual)*100)~=0);
    ...
    %     end
    t_start_idx = find(TM.(t).idata.t < 0.0001, 1, 'last') + 1
    t_start = MC.(t).itime(t_start_idx)

    wspddata = table2array(strides.MC.(t).wspd.normdata);
    strides.(t).pre = find(MC.(t).itime(GE.(t).events.RHS)> (10 + t_start) & MC.(t).itime(GE.(t).events.RHS) < (70 + t_start));
    strides.(t).pert = find(MC.(t).itime(GE.(t).events.RHS)>(70 + t_start) & MC.(t).itime(GE.(t).events.RHS) < (370 + t_start));
    strides.(t).post = find(MC.(t).itime(GE.(t).events.RHS)> (370 + t_start) & MC.(t).itime(GE.(t).events.RHS) < (430+ t_start));
