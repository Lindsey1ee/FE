...
for t = trialTypes
    ...
    tct = tct+1;
    subplot(2,3,tct)
    plot(TM.(t).data.stride,[TM.(t).data.SpeedActual1, TM.(t).data.SpeedActual2]);
    hold on
    plot([1 TM.(t).data.stride(end)],[1.5 1.5], 'k', 'LineWidth',2);
    ylim([0 2.1])
    xlabel("strides"); ylabel("belt speed m/s");
    title(subj+ ": "+ t)
